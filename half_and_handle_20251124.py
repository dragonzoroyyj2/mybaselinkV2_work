...
# 2. 경로 및 상수 설정
# ==============================
BASE_DIR = Path("C:/Test").resolve()  # 💡 강제 지정된 절대 경로
...
# 🔑 [추가된 로직] 하프 앤 핸들 패턴 감지 함수
def find_half_and_handle(df, peaks, troughs, min_duration=60, retrace_ratio=0.3):
    """
    하프 앤 핸들 (컵 형성 초기 단계) 패턴을 감지합니다.
    컵의 왼쪽 벽 형성 후 U자형 바닥을 다지며 상승 전환하는 시점 확인.
    """
    # 최근 250일 데이터의 주요 봉우리와 골짜기만 사용
    recent_troughs = [t for t in troughs if t >= len(df) - 250]
    recent_peaks = [p for p in peaks if p >= len(df) - 250]
    
    if len(recent_troughs) < 1 or len(recent_peaks) < 1:
        return False, None, 'None'

    current_price = df['Close'].iloc[-1]
    
    # 1. 컵의 바닥 (Trough) 인덱스 및 가격 찾기
    trough_idx = recent_troughs[np.argmin(df['Close'].iloc[recent_troughs])]
    trough_price = df['Close'].iloc[trough_idx]
    
    # 2. 컵의 왼쪽 벽 (Left Wall) 찾기: 바닥 이전에 있는 가장 높은 피크
    left_wall_candidates = [p for p in recent_peaks if p < trough_idx]
    if not left_wall_candidates:
         # 바닥 이전에 유의미한 피크가 없으면 컵 패턴 형성으로 보기 어려움
         return False, None, 'None'
    
    # 왼쪽 벽을 바닥 이전에 있는 최고점으로 설정
    left_wall_idx = left_wall_candidates[np.argmax(df['Close'].iloc[left_wall_candidates])]
    left_wall_price = df['Close'].iloc[left_wall_idx]
    
    # 3. 기간 조건: 왼쪽 벽부터 바닥까지 충분한 기간 (최소 60일)
    if trough_idx - left_wall_idx < min_duration:
         return False, None, 'None'

    # 4. 깊이 조건: 왼쪽 벽에서 바닥까지 최소 10% 이상 하락
    drop_from_wall = left_wall_price - trough_price
    if drop_from_wall / left_wall_price < 0.10: 
        return False, None, 'None'
        
    # 5. 상승 전환 (Half) 확인: 현재 가격이 바닥에서 충분히 반등했지만 넥라인 미만
    retrace_up = current_price - trough_price
    neckline = left_wall_price 
    
    # 바닥에서 최소 30% 이상 반등했고, 아직 넥라인을 돌파하지 않은 경우
    is_potential_half = (retrace_up / drop_from_wall) >= retrace_ratio and current_price < neckline
    
    if is_potential_half:
        return True, neckline, 'HalfPotential' 

    return False, neckline, 'None'
...
def check_ma_conditions(df, periods, analyze_patterns):
    ...
    if analyze_patterns:
        ...
        # 🔑 하프 앤 핸들 감지 및 결과 추가
        _, hh_neckline, hh_status = find_half_and_handle(df, peaks, troughs)
        ...
        results['pattern_half_and_handle_status'] = hh_status # 🔑 신규 패턴 상태
        results['hh_neckline_price'] = hh_neckline             # 🔑 신규 패턴 넥라인
...
def analyze_symbol(item, periods, analyze_patterns, pattern_type_filter, symbol_filter=None): 
    ...
    try:
        ...
        if pattern_type_filter:
            ...
            # 🔑 하프 앤 핸들 필터 조건 추가: 'HalfPotential'도 잠재적 신호로 간주
            elif pattern_type_filter in ['double_bottom', 'triple_bottom', 'cup_and_handle', 'half_and_handle']:
                status_key = f'pattern_{pattern_type_filter}_status'
                status = analysis_results.get(status_key)
                is_match = status in ['Breakout', 'Potential', 'HalfPotential']
...
def run_analysis(workers, ma_periods_str, analyze_patterns_flag, pattern_type_filter, top_n, symbol_filter=None): 
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(future_to_item):
            ...
            # 🔑 진행 상황 JSON 출력 (사용자가 화면에서 퍼센트 진행율을 표시하기 위해 필요)
...
def generate_chart(symbol, ma_periods_str, chart_period):
    ...
    try:
        ...
        # 🔑 하프 앤 핸들 넥라인 감지
        _, hh_neckline, hh_status = find_half_and_handle(df_full, peaks_all, troughs_all)
        ...
        patterns_to_check = [
            ...
            ("HalfAndHandle", hh_neckline, hh_status) # 🔑 하프 앤 핸들 추가
        ]
...
def main():
    ...
    parser.add_argument("--pattern_type", type=str,
                        choices=['ma', 'all_below_ma', 'double_bottom', 'triple_bottom', 'cup_and_handle', 'half_and_handle', 'goldencross', 'deadcross', 'regime:0', 'regime:1', 'regime:2', 'regime:3'], # 🔑 'half_and_handle' 추가
                        help="분석 모드에서 필터링할 패턴 종류 (예: goldencross, regime:0, half_and_handle)")
