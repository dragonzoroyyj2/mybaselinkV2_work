# -*- coding: utf-8 -*-
"""
📘 athena_k_market_ai_prod.py (v1.1 - All Patterns Re-Activated & Chart Logic Stabilized)
--------------------------------------------
✅ 한국 주식 시장 데이터 분석 및 기술적 패턴 감지 스크립트
    - 주요 수정: 이전 요청에 따라, Long Term Down Trend 외 3가지 패턴 로직 (이중/삼중 바닥, 컵앤핸들) 재통합 및 활성화.
    - 안정화: 차트 생성 시 패턴 넥라인 정보가 JSON에 포함되도록 로직 복구.
    - **[수정 사항]**: 차트 데이터 출력 시 날짜 형식을 **한국식 ('%Y년 %m월 %d일')**로 변경.
"""
...
class CustomJsonEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        ...
        if isinstance(obj, (pd.Timestamp, datetime, np.datetime64)):
            # 분석 결과 JSON의 날짜 포맷은 그대로 유지 (YYYY-MM-DD)
...
# ==============================
# 5. 기술적 분석 패턴 로직 (하락 추세 및 상승 반전 패턴)
# ==============================
...
def find_peaks_and_troughs(df, prominence=0.01, width=3):
    ...
    # 전체 데이터프레임 인덱스로 변환 (df_analyze 기준 인덱스)
...
def find_double_bottom(df, troughs, tolerance=0.05, min_duration=30):
    """이중 바닥 패턴 감지"""
    # 최근 250일 내의 골짜기만 사용 (이미 find_peaks_and_troughs에서 범위 지정됨)
    ...
    # 잠재적 패턴 확인 (바닥에서 50% 이상 회복, 넥 라인 돌파 전)
...
def find_cup_and_handle(df, peaks, troughs, handle_drop_ratio=0.3):
    ...
    # 컵 모양 확인은 복잡하므로, 여기서는 오른쪽 봉우리를 넥 라인으로 가정
    neckline = peak_right_price
    
    # 핸들 형성 조건: 오른쪽 봉우리 이후 가격이 그 봉우리를 넘지 않고, 
    # 핸들 최대 하락 폭(30%) 이내에 있어야 함
    handle_start_idx = peak_right_idx 
    handle_max_drop = peak_right_price * (1 - handle_drop_ratio) 
    current_price = df['Close'].iloc[-1]
    
    # 오른쪽 봉우리 이후 가격이 그 봉우리를 넘지 않았는지 확인 (현재가를 제외한 최근 며칠간)
    recent_max = df['Close'].iloc[handle_start_idx:-1].max() if len(df) > handle_start_idx + 1 else peak_right_price
    ...
    try:
        ...
        if pattern_type_filter:
            ...
            # ⭐⭐⭐ [수정] 상승 반전 패턴 필터링 로직 재통합 ⭐⭐⭐
                ...
                # 주가가 20, 50, 200일 MA 아래에 있는지 확인
                ma_columns = {20: 'SMA_20', 50: 'SMA_50', 200: 'SMA_200'}
                is_match = all(
                    (df_analyze['Close'].iloc[-1] < df_analyze.get(ma_columns[p], 999999999).iloc[-1])
...
def generate_chart(symbol, ma_periods_str, chart_period):
    ...
    try:
        ...
        for index, row in df_for_chart.iterrows():
            ohlcv_data.append({
                "x": index.strftime('%Y년 %m월 %d일'),  # ⭐ [수정] 한국식 날짜 형식 적용
        ...
        for p in periods:
            ...
            for index, row in df_for_chart.iterrows():
                if not pd.isna(row[ma_col_name]):
                    ma_values.append({"x": index.strftime('%Y년 %m월 %d일'), "y": row[ma_col_name]}) # ⭐ [수정] 한국식 날짜 형식 적용
            ma_data[f"MA{p}"] = ma_values
            
        # 3. MACD 데이터 포맷팅
        macd_data = {"MACD": [], "Signal": [], "Histogram": []}
        for index, row in df_for_chart.iterrows():
            date_str = index.strftime('%Y년 %m월 %d일') # ⭐ [수정] 한국식 날짜 형식 적용
        ...
        if ma50_col in df_for_chart.columns and ma200_col in df_for_chart.columns:
            ...
            for date, is_above in cross_points.items():
                ...
                if cross_type:
                    cross_data.append({"x": date.strftime('%Y년 %m월 %d일'), "y": df_for_chart.loc[date, 'Close'], "type": cross_type}) # ⭐ [수정] 한국식 날짜 형식 적용
        ...
        for p_type, (status, price) in patterns.items():
            if status in ['Breakout', 'Potential'] and price is not None:
                # Chart.js 주석을 위한 간소화된 데이터 추가
                pattern_data.append({
                    "x": df_full.index[-1].strftime('%Y년 %m월 %d일'), # ⭐ [수정] 한국식 날짜 형식 적용
                    "y": price, 
                    "type": p_type, 
                    "status": status,
                    "is_neckline": True # 넥라인 가격임을 표시
                })
        ...
        final_output = {
            ...
            "pattern_points": pattern_data # 패턴 넥라인 및 상태 정보 포함
        }
